...
#### 与 Bitcoin SHA-256d 对比

| 特性 | Bitcoin (SHA-256d) | QFC (Blake3) |
|------|-------------------|--------------|
| 哈希函数 | SHA-256 双重哈希 | Blake3 单次 |
| 计算公式 | `SHA256(SHA256(header))` | `blake3(seed \|\| addr \|\| nonce)` |
| 速度 | 较慢 | 快 3-5 倍 |
| ASIC 友好度 | 高度专业化 | CPU 友好 |
| 能耗 | 极高 | 较低 |
| 安全性 | 经过 15 年验证 | 现代密码学设计 (2020) |
| PoW 权重 | 100% (纯 PoW) | 20% (PoC 多维度) |

#### Bitcoin 使用 SHA-256d 的原因

1. **防长度扩展攻击** - SHA-256 单次存在 length extension 漏洞，双重哈希消除此风险
2. **2009 年的最佳选择** - 当时最成熟、最安全的哈希函数
3. **NIST 标准** - 政府级别密码学认证
4. **100% 算力决定** - ASIC 专业化是合理的经济选择

#### QFC 选择 Blake3 的原因

1. **高性能** - 比 SHA-256 快 5-10 倍，降低计算成本
2. **代码复用** - QFC 全链已使用 Blake3 作为哈希函数
3. **现代设计** - 2020 年发布，吸收了 SHA-3 竞赛的密码学经验
4. **CPU 友好** - 不希望 ASIC 垄断，因为计算贡献只占总评分的 20%
5. **无需双重哈希** - Blake3 设计上不存在长度扩展漏洞
