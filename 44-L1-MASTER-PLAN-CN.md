...
[English](./44-L1-MASTER-PLAN-EN.md) | **中文**
...
- **2026-07-05（周更 #1）**：
  - **Phase 0**：0.1+0.2 完成——合并式审查把剩余缺陷一次找全，实际为 **D6–D12 共 7 个**（不止已知的第 6 个），qfc-core PR #130 合并（确定性执行、自包含验证、fork choice、sync-before-produce 门控、vote-once、finality 加固；ADR-0012 + 16 条 review 修复）。#131 把 qfc-watchdog 打进镜像（T6 sidecar 用）。staging 镜像构建中。**下一步：0.3 reset #3 → 0.4 七天 soak → 0.5 部署 T2–T8 + 合约重部署**。测试网当前仍跑 v2.2.3 旧镜像（分叉中，无害）。
  - **Phase 1**：1.1+1.2 完成——模型 v3（PR #48，条件性通过维持，最差保本币价 $2.11）；45 号主网参数草案 + 42 号 T+12 判定归档（PR #49）。§6 冲突清单 7 个不一致中 2 个是 qfc-core 实现 bug（slashing 三套定义、1% 质押上限死代码），已核实在 #130 后仍存在，issue 待开。**Phase 1 唯一剩余：1.3 发外联约外部 review（人工项）**。
  - **Phase 2 提前项**：2A.1 完成——install.ps1 下载源修正 + 双脚本版本漂移警告（qfc-miner PR #10，对真实 RPC 验证过，正确检出 v2.3.2 vs v2.2.3 漂移）。E2E clean-VPS 验证按计划留待 2A.2（reset 后）。
  - **风险/提醒**：① staging 镜像构建 #130 那次结论是 failure（疑为已知 dispatch step 问题、镜像本身可能正常）——reset 前必须确认镜像真实可用；② v3 模型可能高估矿工费收入 1.43×（按 100% 计，协议实付 70%），已列 45 号 OQ-3，reset 后重跑快照时一并修正。
- **2026-07-06（增补——07-05 周更写于 reset 之前，此后 24 小时进展密集，据实补记）**：
  - **Phase 0 里程碑：0.3 完成，0.4 进行中**。reset #3 于 07-05 执行（镜像 staging-sha-cd5bafd = #130+#131），**三个 validator 历史首次收敛**——block 100 与 tip-5 跨节点哈希逐字节一致（独立验证），50+ 区块零拒绝。**七天 soak 至 ~07-12**。SRE T2–T8 同日部署（watchdog sidecar、SLO 告警上线），当日告警 12→0 清零。
  - **soak 期修复（全部已合并且部署，现行镜像含全部）**：#134 质押上限 20% 强制 + slashing 常量单一来源（关 issue #132/#133，review 两条提醒均已处理）；#136 mempool 导入驱逐 + 周期过期清理（关 #135——僵尸交易致 oldest-tx-age 告警永久红，修后实测 45min→0.5min）；#137 幽灵收据 + reorg 清理、#138 CREATE 地址 nonce、#139 EVM timestamp/gas——三者为 **0.5 合约重部署**扫清执行层障碍。
  - **运维**：explorer 内存告警根因修复（qfc-explorer #124：真 404 渲染 + robots 挡深分页爬虫；旧链 URL 爬虫流量是内存驱动因素）；Grafana T2 面板 datasource 修复（qfc-testnet #6）。
  - **文档**：44/45 英文版（#52）+ 全库 11 对双语补齐（#53）；41 号已标注合约地址失效警告，待 0.5 重部署后刷新。
  - **下一步**：0.4 soak 观察至 07-12（建议补跨节点头哈希分叉告警）→ 0.5 合约全量重部署 + 前端 env/41 号文档更新 → 过 Phase 0 出口门。Phase 1 仍只欠 1.3 人工外联。
